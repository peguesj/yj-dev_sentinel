...
class VCLAAdapter(MCPAgentAdapter):
    ...
    async def _start_monitoring(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hasattr(self.agent, 'start_monitoring'):
            result = await self.agent.start_monitoring(context)
    ...
    async def _stop_monitoring(self) -> Dict[str, Any]:
        """Stop monitoring."""
        if hasattr(self.agent, 'stop_monitoring'):
    ...
    async def _get_listening_status(self) -> Dict[str, Any]:
        ...
        if hasattr(self.agent, 'get_listening_status'):
            status = await self.agent.get_listening_status()
            return {"status": "success", "listening_status": status}
        else:
